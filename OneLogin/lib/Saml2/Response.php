    ...
    public function isValid($requestId = null)
    {
        ...
        try {
            ...
            if ($this->_settings->isStrict()) {
                ...
                $allowed_issuers = array($idPEntityId);
                $allowed_issuers = \Wpo\Services\Saml2_Service::merge_allowed_issuers($allowed_issuers);
                $issuer_tenant_id = \Wpo\Services\Saml2_Service::get_tenant_id_from_entity_id($idPEntityId);

                foreach ($issuers as $issuer) {
                    $trimmedIssuer = trim($issuer);

                    if (!empty($trimmedIssuer) && in_array($trimmedIssuer, $allowed_issuers)) {
                        $issuer_tenant_id = \Wpo\Services\Saml2_Service::get_tenant_id_from_entity_id($trimmedIssuer);
                        break;
                    }
            ...
            if (empty($signedElements) || (!$hasSignedResponse && !$hasSignedAssertion)) {
                ...
                $validate_signature = function ($force = false) use ($issuer_tenant_id, $idpData, $hasSignedResponse, $hasSignedAssertion, $this_class, &$validate_signature) {
                    $cert = \Wpo\Services\Saml2_Service::get_signing_certificate_for_tenant($issuer_tenant_id, $force);
                    ...
                    if ($hasSignedResponse && !OneLogin_Saml2_Utils::validateSign($this_class->document, $cert, $fingerprint, $fingerprintalg, OneLogin_Saml2_Utils::RESPONSE_SIGNATURE_XPATH, $multiCerts)) {
                    ...
                    $documentToCheckAssertion = $this_class->encrypted ? $this_class->decryptedDocument : $this_class->document;
